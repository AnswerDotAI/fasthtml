...
def get(auth):
    ...
    # We can include the todo objects directly as children of the `Form`, because the `Todo` class has `__xt__` defined.
    # This is automatically called by FastHTML to convert the `Todo` objects into `XT` objects when needed.
...
def post(id:list[int]):
    ...
    # Therefore, we return the list of todos, now in the correct order, which will be auto-converted to XT for us.
...
def delete(id:int):
    ...
    # Note that we are not returning *any* XT component that doesn't have an "OOB" swap, so the target element
